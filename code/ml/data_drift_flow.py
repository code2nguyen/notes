...
def calculate_metrics(i):
    ...
    raw_data_slice = raw_data[(raw_data.lpep_pickup_datetime >= (begin_date + datetime.timedelta(i))) & (raw_data.lpep_pickup_datetime < (begin_date + datetime.timedelta(i + 1)))]
    ...
    raw_data_slice['prediction'] = model.predict(raw_data_slice[num_features + cat_features].fillna(0))

    # Calculate the monitoring metrics for the current data
    report.run(reference_data=reference_data, current_data=raw_data_slice, column_mapping=column_mapping)
